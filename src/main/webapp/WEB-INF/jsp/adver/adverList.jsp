    ...
    function initTable(){
        $('#adverTable').bootstrapTable({
            ...
            columns:[
                ...
                {field:'crud',title:'操作',width:150,
                    formatter: function (value,row,index){
                        return '<input type="button" class="btn btn-success" value="新增" onclick="addAdver()">' +
                           /* '<a href="javascript:findAdverSeat("+row.adverid+")">设置广告位</a>' +*/
                            '         ' +
                            '<input type="button" class="btn btn-success" value="设置广告位" onclick="addAdverSeat()">';
                    }
                }
            ],
        });
